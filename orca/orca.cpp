...
int n,m; // n = number of nodes 点, m = number of edges 边
int *deg; // degrees of individual nodes 度
PAIR *edges; // list of edges 边列表

int **adj; // adj[x] - adjacency list of node x 节点x的邻居列表
PII **inc; // inc[x] - incidence list of node x: (y, edge id) 节点x的邻居元组（节点，节点id）列表
bool adjacent_list(int x, int y) { return binary_search(adj[x],adj[x]+deg[x],y); } //判断是否为邻居
int *adj_matrix; // compressed adjacency matrix 邻接矩阵
...
void count4() {
	...
	for (int x=0;x<n;x++) {
		...
		for (int nx=0;nx<deg[x];nx++) {
			...
			//对每个节点的二阶邻居，如果二阶邻居彼此之间存在边，则C4中本节点、此一阶邻居、两个有边的二阶邻居都++
			for (int i=0;i<nn;i++) {
				int z = neigh[i];
	...
	for (int x=0;x<n;x++) {
        ...
        //计算节点轨迹，节点x的轨迹[0]为度数
		orbit[x][0]=deg[x];
		// x - middle node x为中间的节点的情况
		for (int nx1=0;nx1<deg[x];nx1++) {
		    //对于节点x的一阶邻居y和边的ID ey
			int y=inc[x][nx1].first, ey=inc[x][nx1].second;
			for (int ny=0;ny<deg[y];ny++) {
			    //对于一阶邻居y的一阶邻居（即节点x通过y的二阶邻居）z和边ID ez
				int z=inc[y][ny].first, ez=inc[y][ny].second;
				//如果节点x和z有边（即出现三角形）
				if (adjacent(x,z)) { // triangle
					if (z<y) {
						f_12_14 += tri[ez]-1;//加上边ID:ez的公共邻居数-1
						f_10_13 += (deg[y]-1-tri[ez])+(deg[z]-1-tri[ez]);//（一阶邻居y的度-1-边ID:ez的公共邻居数）+（二阶邻居z的度-1-边ID:ez的公共邻居数）
					}
				} else {
			...
			for (int nx2=nx1+1;nx2<deg[x];nx2++) {
			    //对于节点x排在nx1之后的一阶邻居z和边的ID ez
				int z=inc[x][nx2].first, ez=inc[x][nx2].second;
				//如果节点nx1处的节点和之后的一阶邻居无边（和节点x构成三角形）
				if (adjacent(y,z)) { // triangle
					orbit[x][3]++;//orbit节点x的[3]处++
					f_13_14 += (tri[ey]-1)+(tri[ez]-1);//（边ID:ey的公共邻居数-1）+（边ID:ez的公共邻居数-1）
					f_11_13 += (deg[x]-1-tri[ey])+(deg[x]-1-tri[ez]);//（x的度-1-边ID:ey的公共邻居数）+（x的度-1-边ID:ez的公共邻居数）
				} else { // path //如果节点nx1处的节点和之后的一阶邻居无边，则此处是一个路径
					orbit[x][2]++;//orbit节点x的[2]处++
					f_7_11 += (deg[x]-1-tri[ey]-1)+(deg[x]-1-tri[ez]-1);
					f_5_8 += (deg[y]-1-tri[ey])+(deg[z]-1-tri[ez]);
				}
			}
		}
		// x - side node x为边缘节点的情况
		for (int nx1=0;nx1<deg[x];nx1++) {
			int y=inc[x][nx1].first, ey=inc[x][nx1].second;
			for (int ny=0;ny<deg[y];ny++) {
			    //对于每一个节点x关于一阶邻居产生的二阶邻居y和边ID ez来说
				int z=inc[y][ny].first, ez=inc[y][ny].second;
				if (x==z) continue;
				if (!adjacent(x,z)) { // path  //节点x和二阶邻居无边，构成的是路径
					orbit[x][1]++; //orbit节点x的[1]处++
